 ...
 * Description: Imports locations from WP XML export to wpsl_stores with ACF field mapping. Includes dry run, live progress, and prevents duplicate image uploads.
 * Version: 1.0.0
...
class WPSL_Custom_XML_Importer
{
	...
	/**
	 * Class constructor. Hooks into WordPress actions.
	 */
...
?>
		<div class="wrap">
			...
			<p>Upload the XML export file. The importer will match existing <code>wpsl_stores</code> by slug, parse Gutenberg blocks and Meta values, map them to your ACF JSON structure, and securely handle image sideloading.</p>
					...
					$.ajax({
						...
						success: function(response) {
							if (response.success) {
								...
								processBatch(0, 3, response.data.total, dryRun); // Batch size 3 to avoid image download timeouts
							} else {
...
<?php
	...
	/**
	 * AJAX handler: Uploads XML, iterates to map attachments and extracts 'locations' to a temporary map JSON file.
	 */
	public function ajax_upload_xml()
	{
		...
		// Increase boundaries for dense XML structures
		...
		foreach ($xml->channel->item as $item) {
			...
			if ($post_type === 'attachment') {
				$attachments[(string) $ns_wp->post_id] = (string) $ns_wp->attachment_url;
		...
		// Output mappings locally to sustain batching via AJAX
	...
	/**
	 * Core Parsing Engine: Executes block abstractions and maps target data to the corresponding ACF fields.
	...
	private function process_location($loc, $attachments, $dry_run)
	{
		...
		if ($existing) {
			...
			$log .= "    [Info] Pre-existing record identified: ID {$post_id}. Executing overwrite...\n";
			if (! $dry_run) {
				wp_update_post([
					'ID'           => $post_id,
					'post_title'   => $title,
		...
		// 1. Process Extant Metadatas first
		...
		// Deconstruct serialized metadata payload (Common in preceding layouts)
		...
		// 2. Parse inner Gutenberg JSON architectures (For modern variants like Knollbeck & Blamsters)
		// Check for CQC widgets inside the encoded block markup
				...
				if (isset($data['data'])) {
					foreach ($data['data'] as $k => $v) {
						if (strpos($k, 'item_title') !== false) {
							$val = trim($v);
							// RegEx isolate numerical parameters inside facility labels
							...
							// Enforce strict categorizations targeting ACF `our_expertise` choices payload vs general facilities
		...
		// Identify image integrations wrapped inside `content-cards` blocks
		...
		// 3. Inject ACF Meta-References using secure mapping directives utilizing the explicit JSON exported keys 
		if (! $dry_run) {
			// Arrays: facilities_and_features
			update_field('field_69aa920b6893c', array_unique($facilities), $post_id);

			// Arrays: our_expertise
			update_field('field_69aa958818e50', array_unique($expertise), $post_id);

			if ($bedrooms) {
				update_field('field_69aa9338e1174', $bedrooms, $post_id);
			}
			if ($ensuite) {
				update_field('field_69c3c065ee56d', $ensuite, $post_id);
			}
			if ($cqc_id) {
				update_field('field_69aaa93fb0482', $cqc_id, $post_id);
			}

			// Universal gallery injections. Maps the sideloaded library array into target meta layers.
			if (! empty($gallery_ids)) {
				$unique_gallery = array_unique($gallery_ids);
				update_post_meta($post_id, 'gallery', $unique_gallery);
			}

			// Map thumbnail correctly based on meta extraction layer if declared
		...
		if ($dry_run) {
			$log .= "    [DRY RUN] ACF fields conditionally parsed but preserved without database alterations.\n\n";
		} else {
	...
	private function sideload_image($url)
	{
		...
		// Restructure protocolless imports logically
		if (strpos($url, '//') === 0) {
			$url = 'https:' . $url;
		}

		// Determine pure string filename mapping
		$filename = basename(parse_url($url, PHP_URL_PATH));

		global $wpdb;

		// Perform strict SQL evaluation looking for duplicate image paths within `_wp_attached_file`
		$query = $wpdb->prepare(
		...
		if ($existing) {
			return (int) $existing; // Return native library ID, avoiding redundantly downloading it
		}
