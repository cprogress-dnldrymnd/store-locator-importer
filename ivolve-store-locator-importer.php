		...
		private static function extractAllAcfBlockJson(string $postContent, string $blockSlug): array
		{
			...
			$extractParagraphs = function (string $segment, bool $truncateAtFirstButton = true): array {
				if ($truncateAtFirstButton) {
					// Locate standard WordPress block button or generic HTML button
					$buttonPos = strpos($segment, 'wp-block-button');
					if ($buttonPos === false) {
						$buttonPos = strpos($segment, '<button');
					}

					// Isolate the text before the button
					$beforeButton = ($buttonPos !== false) ? substr($segment, 0, $buttonPos) : $segment;

					// Perform the regex match on the truncated substring
					if (! preg_match_all('/<p[^>]*>(.*?)<\/p>/s', $beforeButton, $matches)) {
						return [];
					}
