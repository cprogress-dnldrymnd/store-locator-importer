...
if (! class_exists('IVolve_WXR_Store_Locator_Parser')) {
	class IVolve_WXR_Store_Locator_Parser
	{
		...
		public function __construct($wxrPath)
		{
		...
		public function iterateLocations(callable $onLocation): void
		{
			...
			foreach ($xml->channel->item as $item) {
				...
				$postStatus =  ($wp->status ?? 'publish');
				$postId = (int) ($wp->post_id ?? 0);

				$postTitle =  ($item->title ?? $postName);

				$contentNs = $item->children('content', true);
				$postContent =  ($contentNs->encoded ?? '');
...
if (! class_exists('IVolve_Store_Locator_Import_Command')) {
	class IVolve_Store_Locator_Import_Command
	{
		...
		{
			$postContent =  ($location['post_content'] ?? '');
			...
			$locationArr = is_($serializedLocation) ? @unserialize($serializedLocation) : false;
			if (is_array($locationArr)) {
				$streetNumber =  ($locationArr['street_number'] ?? '');
				$streetName =  ($locationArr['street_name'] ?? '');
				$wpsl['wpsl_address'] = trim($streetNumber . ' ' . $streetName);
				$wpsl['wpsl_city'] =  ($locationArr['city'] ?? '');
				$wpsl['wpsl_zip'] =  ($locationArr['post_code'] ?? '');
				$wpsl['wpsl_country'] =  ($locationArr['country'] ?? '');
			}
			...
			$twoColumnsHeading =  ($twoImageColumns['second']['heading'] ?? '');
			if (trim($twoColumnsHeading) === '') {
				$twoColumnsHeading =  ($twoImageColumns['first']['heading'] ?? '');
			}
			$twoColumnsContent =  ($twoImageColumns['second']['content'] ?? '');
			if (trim($twoColumnsContent) === '') {
				$twoColumnsContent =  ($twoImageColumns['first']['content'] ?? '');
			}
			...
			if (! empty($catsByDomain['location_type']) && is_array($catsByDomain['location_type'])) {
				$taxonomyCategory =  ($catsByDomain['location_type'][0] ?? '');
			}

			$locationDescription =  ($meta['location_description'] ?? '');
			...
			return [
				...
				'bedrooms' => $bedrooms,
				'cqc_id' => $cqcId,
				'first_section_heading' => ($twoImageColumns['first']['heading'] ?? ''),
				'first_section_content' => ($twoImageColumns['first']['content'] ?? ''),
				...
				'two_columns_button_text' => ($twoColumnsButton['text'] ?? ''),
				// Elementor's ACF_URL / href sanitization expects a  URL, not the full Link array.
				'two_columns_button_link' => ($twoColumnsButton['link']['url'] ?? ''),
				'walkthrough_360' =>  $walkthrough360,
		 ...
		 */
		private static function extractAllAcfBlockJson($postContent,  $blockSlug): array
		{
			$out = [];
			$needle = '<!-- wp:' . $blockSlug;
			$offset = 0;
		...
		private static function extractPageHeaderHeroImageAttachmentId($postContent): ?int
		{
		 ...
		 *
		 * @return array<, mixed>|null { 'text' => , 'link' => array{url:,title:,target:} }
		 */
		private static function extractTwoColumnsButtonFromContent($postContent): ?array
		{
		...
		private static function extractWalkthrough360FromContent($postContent): 
		{
		...
		private static function findMatchingBrace($s, int $startIndex): int
		{
			$depth = 0;
			$in = false;
			$escape = false;
		 ...
		 *
		 * @return array{
		 *   first: array{heading:, content:, image_old_attachment_id:?int},
		 *   second: array{heading:, content:, image_old_attachment_id:?int}
		 * }
		 */
		private static function extractImageColumnSections($postContent): array
		{
			...
			$extractHeading = function ($segment):  {
			...
			$extractParagraphs = function ($segment, bool $truncateAtFirstButton = true): array {
			...
			$extractOldAttachmentRef = function (array $jsonData,  $segment) {
				$candidates = [
					'image',
					'image_id',
					'imageId',
		...
		private static function extractListIconTitles($postContent): array
		{
			$out = [
				'our_expertise' => [],
			...
			foreach ($blocks as $idx => $block) {
				...
				if ($n > 0) {
					...
					foreach ($data as $k => $_v) {
						if (preg_match('/^list_items_(\d+)_item_title$/',  $k, $m)) {
							$indexes[] = (int) $m[1];
						}
					}
					sort($indexes);
		 ...
		 * @return array<int, >
		 */
		private static function normalizeCheckboxValues($fieldName, array $items): array
		{
		 ...
		 *
		 * @return array<, mixed> map of normalized  -> stored value
		 */
		private static function buildCheckboxChoiceValueMap($fieldName): array
		{
			...
			foreach ($choices as $value => $label) {
				$valueNorm = strtolower(preg_replace('/\s+/u', ' ', trim($value)));
				$labelNorm = strtolower(preg_replace('/\s+/u', ' ', trim($label)));
		 ...
		 * @return array<int, >
		 */
		private static function getCheckboxAllowedValues($fieldName): array
		{
		...
		/**
		 * @return array<int, array{heading:, old_attachment_id:int|null}>
		 */
		private static function extractContentCards($postContent): array
		{
			...
			if ($cardsCount > 0) {
				for ($i = 0; $i < $cardsCount; $i++) {
					$heading = isset($data['cards_' . $i . '_card_heading']) ?  $data['cards_' . $i . '_card_heading'] : '';
			...
			$indexes = [];
			foreach ($data as $k => $_v) {
				if (preg_match('/^cards_(\d+)_card_image$/',  $k, $m)) {
					$indexes[] = (int) $m[1];
				}
			}
			sort($indexes);
		 ...
		 *
		 * @param array<int, array{heading:, old_attachment_id:int|null}> $cards
		 * @return array<, mixed>
		 */
		private static function buildGalleryFromCards(array $cards): array
		{
			...
			$findByNeedle = function ($needle,  $mustNot = '') use ($cards, &$usedAttachmentIds): ?int {
		...
		private static function getExistingStoreIdBySlug($slug): int
		{
			$posts = get_posts(
				[
					'name' => $slug,
		...
		private static function getImportSlug(array $location): 
		{
			$postName = trim(($location['post_name'] ?? ''));
			if ($postName !== '') {
				return $postName;
			}
		...
		private static function mapOldStatusToTargetStatus(array $location): 
		{
			$old = strtolower(trim(($location['post_status'] ?? '')));
			if ($old === 'draft') {
				return 'draft';
			}
			return 'publish';
		...
		private static function updateFieldIfNeeded(int $storeId,  $fieldName, $value, bool $isExisting,  $merge): void
		{
		...
		private static function shouldUpdateField(int $storeId,  $fieldName, bool $isExisting,  $merge): bool
		{
			if (! $isExisting) {
				return true;
			}
			...
			if (! $isEmpty && is_($current)) {
				$trimmed = trim($current);
				$isEmpty = in_array($trimmed, ['[]', 'a:0:{}'], true);
				// If a previous run stored a wrong Link array into a field that the template expects as URL ,
			...
			if (! $isEmpty && $fieldName === 'two_columns_heading') {
				...
				if (is_($current) && $current !== '' && is_($firstHeading) && trim($current) === trim($firstHeading)) {
					$isEmpty = true;
				}
			}
		...
		private static function doUpdateField(int $storeId,  $fieldName, $value): void
		{
		...
		/**
		 * @param array<int, > $attachmentUrlMap old attachment id => url
		 * @param array<, int> $uploadedCache cache old id/url => new attachment id
		 * @param int||null $oldAttachmentRef old attachment id (int) OR old attachment URL 
		 */
		private static function sideloadOldAttachmentId($oldAttachmentRef, array $attachmentUrlMap, array &$uploadedCache, int $storeId, bool $dryRun, array &$report): ?int
		{
			...
			$uploadedCache[$cacheKey] = (int) $newId;
			$report['images_sideloaded'] = (int) ($report['images_sideloaded'] ?? 0) + 1;
			return (int) $newId;
		}

		private static function updatePostMetaIfNeeded(int $storeId,  $metaKey, $value, bool $isExisting,  $merge): void
		{
			if (! $isExisting) {
		...
		private static function ensureWpslCategory(int $storeId,  $nicename): void
		{
			if (! $nicename) {
				return;
			}
		 ...
		 *
		 * @return array<,mixed> report counters
		 */
		public static function importFromOldXml($oldXml,  $slug,  $merge, bool $dryRun): array
		{
			...
			$parser->iterateLocations(
				function (array $location) use ($slug, $dryRun, $merge, $attachmentUrlMap, &$uploadedAttachmentIdCache, &$report) {
					...
					if (! $isExisting) {
						$createdId = wp_insert_post(
							[
								...
								'post_title' => ($location['post_title'] ?? $oldPostName),
						...
						self::ensureWpslCategory($storeId, ($payload['taxonomy_category_nicename'] ?? ''));
					...
					foreach ((array) ($payload['wpsl'] ?? []) as $metaKey => $metaValue) {
						self::updatePostMetaIfNeeded((int) $storeId,  $metaKey, $metaValue, $isExisting, $merge);
					}

					// ACF fields.
					self::updateFieldIfNeeded((int) $storeId, 'number_of_bedrooms', (int) ($payload['bedrooms'] ?? 0), $isExisting, $merge);
					self::updateFieldIfNeeded((int) $storeId, 'cqc_id', ($payload['cqc_id'] ?? ''), $isExisting, $merge);

					self::updateFieldIfNeeded((int) $storeId, 'first_section_heading', ($payload['first_section_heading'] ?? ''), $isExisting, $merge);
					self::updateFieldIfNeeded((int) $storeId, 'first_section_content', ($payload['first_section_content'] ?? ''), $isExisting, $merge);
					self::updateFieldIfNeeded((int) $storeId, 'two_columns_heading', ($payload['two_columns_heading'] ?? ''), $isExisting, $merge);
					self::updateFieldIfNeeded((int) $storeId, 'two_columns_content', ($payload['two_columns_content'] ?? ''), $isExisting, $merge);

					self::updateFieldIfNeeded((int) $storeId, 'two_columns_button_text', ($payload['two_columns_button_text'] ?? ''), $isExisting, $merge);
					self::updateFieldIfNeeded((int) $storeId, 'two_columns_button_link', ($payload['two_columns_button_link'] ?? ''), $isExisting, $merge);
					self::updateFieldIfNeeded((int) $storeId, 'walkthrough_360', ($payload['walkthrough_360'] ?? ''), $isExisting, $merge);
					...
					foreach ($imageFields as $fieldName => $oldAttachmentId) {
						if (! self::shouldUpdateField((int) $storeId,  $fieldName, $isExisting, $merge)) {
							continue;
						}
						...
						if ($newAttachmentId) {
							self::doUpdateField((int) $storeId,  $fieldName, (int) $newAttachmentId);
					...
					for ($i = 1; $i <= 10; $i++) {
						...
						$slotText = isset($galleryTexts[$imgIndex]) ?  $galleryTexts[$imgIndex] : '';
					...
					if ($desc) {
						...
						if (! $isExisting) {
							...
							$currentContent =  get_post_field('post_content', (int) $storeId);
		...
		public static function run(array $args, array $assoc_args): void
		{
			...
			$merge = strtolower(($assoc_args['merge'] ?? 's2'));
			...
			$parser->iterateLocations(
				function (array $location) use ($slug, &$processed, $dryRun, $attachmentUrlMap, $merge, &$uploadedAttachmentIdCache, &$report) {
					...
					if (! $dryRun && ! $isExisting) {
						...
						self::ensureWpslCategory((int) $storeId,  $payload['taxonomy_category_nicename']);
...
if (is_admin()) {
	...
	add_action(
		...
		function () {
			...
			$merge = isset($_POST['merge']) ?  $_POST['merge'] : 's2';
			...
			if (isset($handled['error'])) {
				wp_die('Upload failed: ' . esc_html($handled['error']));
			}

			$oldXmlPath =  ($handled['file'] ?? '');
